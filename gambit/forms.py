...
class SubmitForm(forms.ModelForm):
    ...
    def clean_file(self):
        ...
        """If type(file) == FieldFile, then the file has already been uploaded during submission or a previous edit and does not need to be cleaned. It will throw an AttributeError on the content_type assignment because the model does not have such a field. This may happen when the user tries to change the uploaded file but a ValidationError occurs. If the user then chooses not to select a different file but clicks submit to update the submission, clean_file() will attempt to process the file which is actually a FieldFile causing the error described before.
        """
        if file and type(file) != FieldFile:
